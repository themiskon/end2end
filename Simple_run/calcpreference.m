function[theta, depthDay, depthNight, avlocDay, avlocNight] = calcpreference(param)
   ...
   % zooplankton night
   xloc = 0; 
   zp_n = (1./(sqrt(2*3.14*sigmap(param.ixZoo).^2))) .* ...
            exp(-((xrange-xloc).^2' ./(2*sigmap(param.ixZoo).^2)));
   ...
   theta(idx_dems,param.ixZoo)=  theta(idx_dems,param.ixZoo)*0;
